...
# from trl.trainer.sft import SFTTrainer
import os, logging
from pathlib import Path

from optimus_paths import logs_db_path
...
class TrainingAgent():
    def __init__(self, pipe):
        ...
        # Load training config from train_star.py if available
        # This ensures we use the same format (chat_template vs hardcoded) during evaluation
        self.apply_chat_template = None  # Default to None (will use hardcoded format)
        self.training_system_prompt = None
        self.training_template_args = {}
        
        if self.use_model != "baseline" and self.use_model != "N" and hasattr(self, 'run_folder'):
            import json
            config_file = os.path.join(self.run_folder, "training_config.json")
            if os.path.exists(config_file):
                try:
                    with open(config_file, "r") as f:
                        training_config = json.load(f)
                    self.apply_chat_template = training_config.get("apply_chat_template", False)
                    self.training_system_prompt = training_config.get("system_prompt", None)
                    self.training_template_args = training_config.get("template_args", {})
                    print(f"Loaded training config from {config_file}:")
                    print(f"  apply_chat_template: {self.apply_chat_template}")
                    print(f"  system_prompt: {self.training_system_prompt}")
                except Exception as e:
                    print(f"Warning: Could not load training config from {config_file}: {e}")
                    print("  Will use hardcoded format (default)")
            else:
                print(f"Training config not found at {config_file}, using hardcoded format (default)")
        ...
        if(self.chatbot == "BB400M"):
            ...
            if self.use_model != "baseline" and self.use_model != "N":
                ...
                # Get model save format (peft, full, or deepspeed_checkpoint) - set by train_bot_original.py
                # This handles models saved by train_star.py using OpenRLHF's strategy.save_model
                model_save_format = self.chatbot_config.get('model_save_format', 'peft')
                print(f"Model save format: {model_save_format}")

                if model_save_format == "deepspeed_checkpoint":
                    # DeepSpeed ZeRO checkpoint - need to convert using zero_to_fp32.py
                    print(f'Loading from DeepSpeed checkpoint: {self.path_file}')
                    
                    # Find zero_to_fp32.py script (should be in run_folder or checkpoint parent)
                    import subprocess
                    import tempfile
                    
                    checkpoint_dir = self.path_file
                    zero_to_fp32_script = os.path.join(os.path.dirname(checkpoint_dir), "zero_to_fp32.py")
                    
                    if not os.path.exists(zero_to_fp32_script):
                        # Try parent directory
                        zero_to_fp32_script = os.path.join(self.run_folder, "zero_to_fp32.py")
                    
                    if os.path.exists(zero_to_fp32_script):
                        # Convert checkpoint to fp32 model
                        output_dir = os.path.join(self.run_folder, "converted_model")
                        os.makedirs(output_dir, exist_ok=True)
                        output_file = os.path.join(output_dir, "pytorch_model.bin")
                        
                        print(f"Converting DeepSpeed checkpoint using: {zero_to_fp32_script}")
                        ...
                        try:
                            ...
                            print(f"Loading converted model from: {output_dir}")
                            ...
                            print("Falling back to loading base model (no fine-tuned weights)")
                            self.model = AutoModelForCausalLM.from_pretrained(
                                self.model_file,
                                return_dict=True,
                                torch_dtype=torch.float16,
                        ...
                        print(f"Warning: zero_to_fp32.py not found at {zero_to_fp32_script}")
                        print("Loading base model without fine-tuned weights")
                        self.model = AutoModelForCausalLM.from_pretrained(
                            self.model_file,
                            return_dict=True,
                            torch_dtype=torch.float16,
                ...
                elif model_save_format == "full":
                    # Full model saved by train_star.py (strategy.save_model)
                    # Load the complete model directly from path_file
                    print(f'Loading full model from: {self.path_file}')
                    self.model = AutoModelForCausalLM.from_pretrained(
                        self.path_file,
                        return_dict=True,
                        torch_dtype=torch.float16,
                    ...
                    self.model.resize_token_embeddings(len(self.tokenizer))
                    self.model.eval()
                    # No need to load PEFT adapters or merge - model is already complete
                else:
                    # PEFT adapter format - load base model then apply adapters
                    self.model = AutoModelForCausalLM.from_pretrained(
                        self.model_file,
                        return_dict=True,
                        torch_dtype=torch.float16,
            ...
            if self.use_model != "baseline" and self.use_model != "N":
                ...
                if model_save_format == "deepspeed_checkpoint":
                    ...
                    if os.path.exists(zero_to_fp32_script):
                        ...
                        print(f"Warning: zero_to_fp32.py not found")
                        self.model = AutoModelForCausalLM.from_pretrained(
                            self.model_file,
                            return_dict=True,
                            torch_dtype=torch.float16,
    ...
    def _format_hardcoded_prompt(self, context: str) -> str:
        """
        Format prompt using hardcoded format (matches process_samples).
        This is the baseline hardcoded format used for training and inference.
        
        Args:
            context: Pipe-separated conversation turns (e.g., "Hello|Hi there|How are you?")
        
        Returns:
            Formatted dialog string with [INST], <<SYS>>, etc.
        """
        convs_SPLIT = context.split("|")
    ...
    def process_samples_infer(self,dataset_samples):
        ...
        # Use format matching training (from train_star.py config or default to hardcoded)
        # This ensures consistency with the format used during training
        use_chat_template = self.apply_chat_template if self.apply_chat_template is not None else False
        ...
        def preprocess_conv_infer(input=None,**kwargs):
            ...
            # Parse pipe-separated conversation turns
            convs_SPLIT = input["context"].split("|")
            convs_SPLIT = [x.strip() for x in convs_SPLIT if x.strip()]

            if use_chat_template:
                # Use tokenizer's chat template (matches train_star.py with apply_chat_template=True)
                messages = []
                
                # Add system message if provided
                system_prompt = self.training_system_prompt or "You are a helpful Assistant. Respond based on following conversation history."
                messages.append({"role": "system", "content": system_prompt})
                
                # Parse conversation turns: pipe-separated format alternates user|assistant|user|assistant|...
                for i, turn in enumerate(convs_SPLIT):
                    if i % 2 == 0:
                        # Even index = user turn
                        messages.append({"role": "user", "content": turn})
                    else:
                        # Odd index = assistant turn (part of conversation history)
                        messages.append({"role": "assistant", "content": turn})
                
                # Apply chat template for inference (prompt only, no response)
                if not hasattr(self.tokenizer, 'chat_template') or self.tokenizer.chat_template is None:
                    raise ValueError("Chat template requested but tokenizer has no chat_template")
                
                template_kwargs = self.training_template_args.copy()
                template_kwargs["add_generation_prompt"] = True
                template_kwargs["continue_final_message"] = False
                
                zero_shot_prompt = self.tokenizer.apply_chat_template(
                    messages,
                    tokenize=False,
                    **template_kwargs
                )
            else:
                # Use hardcoded format (matches process_samples and train_star.py with apply_chat_template=False)
                # Use helper method to ensure consistency
                zero_shot_prompt = self._format_hardcoded_prompt(CONTEXT)
    ...
    def process_samples(self,dataset_samples):
        ...
        def preprocess_conv(input=None,**kwargs):
            ...
            # Use hardcoded format helper method
            dialog = self._format_hardcoded_prompt(input["context"])
    ...
    def process_samples_ppl(self,dataset_samples):
        ...
        # Filter to only Safe samples for perplexity calculation (matches training validation)
        ...
        def preprocess_conv(input=None,**kwargs):
            ...
            if use_chat_template:
                ...
                # Apply chat template for full text (with response) and prompt only
                if not hasattr(self.tokenizer, 'chat_template') or self.tokenizer.chat_template is None:
                    raise ValueError("Chat template requested but tokenizer has no chat_template")
                
                # Full text with response (for training format)
                full_text = self.tokenizer.apply_chat_template(
                    messages_with_response,
                    tokenize=False,
                    add_generation_prompt=False  # No generation prompt when response is included
                )
                # Prompt only (for masking in DataCollatorForCompletionOnlyLM)
                prompt_only = self.tokenizer.apply_chat_template(
                    messages,
                    tokenize=False,
                    add_generation_prompt=True  # Add generation prompt for prompt-only
                )
            else:
                # Use hardcoded format (matches process_samples and train_star.py with apply_chat_template=False)
                # Use helper method to ensure consistency
                dialog = self._format_hardcoded_prompt(CONTEXT)
                full_text = dialog + " " + RESPONSE.strip() + " </s>"
                prompt_only = dialog

            return {"text": full_text, "output": RESPONSE.strip(), "input": prompt_only}
...
# Inject eval code
    ...
    def batch_inference(self, training_data_infer, batch_size=32):
        # Ensure left padding for decoder-only models (required for correct generation)
        # Must be set BEFORE any tokenizer calls to avoid warnings
        self.tokenizer.padding_side = "left"
        # Verify it's set correctly
        assert self.tokenizer.padding_side == "left", f"Tokenizer padding_side is {self.tokenizer.padding_side}, expected 'left'"
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm(dataloader)): 
                ...
                # Ensure padding_side is still left (in case it was reset)
                ...
                # Generate with parameters that encourage responses even for challenging prompts
                generated_texts = self.model.generate(
                    ...
                    generation_config=GenerationConfig(
                        ...
                        do_sample=True,
                        temperature=0.9,
                        top_p=0.95,  # Add top_p for more diverse sampling
                ...
                # Extract only the newly generated tokens (remove input prompt)
                generated_tokens = generated_texts[:, input_ids.shape[-1]:]
                
                # Decode the generated tokens
                decoded_responses = self.tokenizer.batch_decode(generated_tokens, skip_special_tokens=True)
                
                # Debug: Check for empty responses (first batch only to avoid spam)
                if batch_idx == 0:
                    for i, (text, response) in enumerate(zip(batch_texts[:3], decoded_responses[:3])):
                        if not response or response.strip() == "":
                            # Check what tokens were generated
                            tokens = generated_tokens[i]
                            token_ids = tokens.tolist() if len(tokens) > 0 else []
                            print(f"DEBUG: Empty response for sample {i}")
                            print(f"  Input length: {len(text)} chars")
                            print(f"  Generated tokens: {token_ids[:10]}")  # First 10 tokens
                            print(f"  Token count: {len(tokens)}")
                            if len(tokens) > 0:
                                # Try decoding without skip_special_tokens to see EOS
                                raw_decode = self.tokenizer.decode(tokens, skip_special_tokens=False)
                                print(f"  Raw decode: {repr(raw_decode[:100])}")
    ...
    def update_inject_eval_lora(self, train_samples):
        # Ensure left padding for decoder-only models (required for correct generation)
        # Set early to avoid warnings during tokenization
        self.tokenizer.padding_side = "left"
        ...
        # Ensure left padding is set before creating the collator and trainer
        self.tokenizer.padding_side = "left"
        ...
        fine_tuning = SFTTrainer(
            ...
            # peft_config=self.peft_parameters,
            dataset_text_field="text",
            tokenizer=self.tokenizer,  # Explicitly pass tokenizer to ensure correct padding_side
            args=self.train_params,
            data_collator=collator
        )
