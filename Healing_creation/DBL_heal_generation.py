...
def preprocess_conv(input=None,**kwargs):
    ...
    if input['implicit'] is None:
        return {"prompt_input":{"prompt": prompts_list },"ground": ground,"convs": dialog,"context": input['context'],"response": input['response'],"category": input['category'],"label": input['label'],"implicit": "Nothing","source": input['source'],"index": input['index'], "uid": input['uid'], "flags": input['flags']}

    return {"prompt_input":{"prompt": prompts_list },"ground": ground,"convs": dialog,"context": input['context'],"response": input['response'],"category": input['category'],"label": input['label'],"implicit": input['implicit'],"source": input['source'],"index": input['index'] , "uid": input['uid'], "flags": input['flags']}
...
def main():
    ...
    path = f"../Datasets/DBL_data/Detect_Files/{args.path}.csv"

    dataset = load_dataset("csv", data_files=path,split='train')

    dataset = dataset.filter(lambda x: x['prediction'] == "Unsafe")
    ...
    dataset.to_json(f'../Datasets/Context_Heal/DBL_data/{args.model.replace("/","_")}_{ts}_category_{args.category}_type_{args.type}_{args.path}_heal_dataset.json')
    ...
    with open(f'../Datasets/Context_Heal/DBL_data/{args.model.replace("/","_")}_{ts}_category_{args.category}_type_{args.type}_{args.path}_heal_dataset.csv', 'a', newline='') as csvfile:
        spamwriter = csv.writer(csvfile,quoting=csv.QUOTE_ALL)

        header = ['context','response','category','label','implicit','source','index','heal_generated', 'uid', 'flags']
        # 'context','response','category','label','implicit','source','index', 'f1', 'f2', 'f3', 'f4', 'f5', 'flags'
        ...
        for i in tqdm(range(0,len(dataset),args.batch_size)):
            ...
            # 'f1', 'f2', 'f3', 'f4', 'f5', 'flags'
            batch_uid = dataset[i:i+args.batch_size]["uid"]
            batch_flags = dataset[i:i+args.batch_size]["flags"]
            ...
            if args.local_rank == 0:
                # add instructionspecific to the dataset\
                    ...
                    for ids,sentence in enumerate(instruction_specific):
                        ...
                        row.append(batch_uid[ids])
                        row.append(batch_flags[ids])
