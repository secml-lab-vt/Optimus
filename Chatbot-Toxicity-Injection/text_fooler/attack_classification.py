...
class NLI_infer_BERT(nn.Module):
    def __init__(self,
        ...
        self.model = BertForSequenceClassification.from_pretrained(pretrained_dir, num_labels=nclasses).to(DEVICE)

        # construct dataset loader
        self.dataset = NLIDataset_BERT(pretrained_dir, max_seq_length=max_seq_length, batch_size=batch_size)

    def text_pred(self, context, text_data, batch_size=32):
        ...
        for input_ids, input_mask, segment_ids in dataloader:
            ...
            with torch.no_grad():
                logits = self.model(input_ids, segment_ids, input_mask)['logits']
...
def attack(text_ls, true_label, predictor, stop_words_set, word2idx, idx2word, cos_sim, sim_predictor=None,
           ...
           batch_size=32, context=None):
    # first check the prediction of the original text
    orig_probs = predictor([context], [text_ls]).squeeze()
    ...
    if true_label != orig_label:
        ...
        leave_1_probs = predictor([context]*len(leave_1_texts),leave_1_texts, batch_size=batch_size)
        ...
        for idx, synonyms in synonyms_all:
            ...
            new_probs = predictor([context]*len(new_texts), new_texts, batch_size=batch_size)
            ...
            if np.sum(new_probs_mask) > 0:
                ...
                if new_label_prob_min < orig_prob:
                    ...
                    num_changed += 1
            text_cache = text_prime[:]
        return ' '.join(text_prime), num_changed, orig_label, torch.argmax(predictor([context],[text_prime])), num_queries
...
class Detoxify_wrapper():
    def __init__(self, device):
        self.model = Detoxify('unbiased', device=device)
        self.device = device

    def text_pred(self, contexts, texts, batch_size=4):
        texts = [" ".join(x) for x in texts]

        x_broken = [texts[i:i+batch_size] for i in range(0, len(texts), batch_size)]
        prediction_scores = []
        for i, batch in enumerate(x_broken):
            #print(f"\r{i}/{len(x_broken)} batchs - ETA:{(len(x_broken)-i-1)*(t1-t0)/(i+1):.2f}", end="")
            prediction_scores.extend(self.model.predict(batch)["toxicity"])
        #print()

        results = [[1-x, x] for x in prediction_scores]
...
def main():
    ...
    parser.add_argument("--target_model",
                        type=str,
                        required=True,
                        choices=['wordLSTM', 'bert', 'wordCNN', 'detoxify'],
    ...
    # get data to attack
    contexts, texts, labels = dataloader.read_corpus(args.dataset_path)
    data = list(zip(contexts, texts, labels))
    ...
    elif args.target_model == 'detoxify':
        model = Detoxify_wrapper(DEVICE)
    else:
    ...
    for idx, (context, text, true_label) in enumerate(data):
        print('\n{} samples out of {} have been finished!'.format(idx, args.data_size))
        ...
        else: #This one seems to be used
            new_text, num_changed, orig_label, \
        ...
        orig_labels.append(orig_label.item())
    ...
    df = pd.DataFrame()
    df['orig_texts'] = orig_texts
    df['orig_labels'] = orig_labels
    df['true_labels'] = true_labels
    df['adv_texts'] = adv_texts
    df['new_labels'] = new_labels
    df['successes'] = [1 if x1 != x2 else 0 for x1, x2 in zip(true_labels, new_labels)]
    df.to_csv(os.path.join(args.output_dir, 'adversaries.csv'))
