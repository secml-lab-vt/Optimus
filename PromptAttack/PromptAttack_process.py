...
def preprocess_conv(input=None,**kwargs):
    ...
    def attack_prompt(x, y, t_a, perturbation_instruction_i, few_shot_example=None):
        # x = [[t_1,c_1],...,[t_n,c_n]]
        ...
        attack_objective += "{} ".format("Safe" if y == "Unsafe" else "Unsafe")
        attack_objective += ". \n"
    ...
    for perturbation_instruction_index,instruction in enumerate(instructions):
        dialog = ""
        dialog = attack_prompt(x, y, t_a, instruction)
...
class PromptAttack_process:
    ...
    def dataset_processor(self, path):
        ...
        dataset = dataset.map(preprocess_conv,fn_kwargs={"instructions": self.perturbation_instruction},remove_columns=dataset.features)
        return dataset
