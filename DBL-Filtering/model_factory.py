...
class ModelFactory():
    ...
    def preprocess_conv(self,input=None,**kwargs):
        ...
        if input['implicit'] is None:
            return {"prompt_input":{"prompt": prompts_list },"ground": ground,"convs": inference_example,"context": input['context'],"response": input['response'],"category": input['category'],"label": input['label'],"implicit": "Nothing","source": input['source'],"index": input['index'], "uid": input['uid'], "flags": input['flags']}

        return {"prompt_input":{"prompt": prompts_list },"ground": ground,"convs": inference_example,"context": input['context'],"response": input['response'],"category": input['category'],"label": input['label'],"implicit": input['implicit'],"source": input['source'],"index": input['index'],  "uid": input['uid'], "flags": input['flags']}
    ...
    def detect(self,dataset):
        ...
        store_path = detect_scores_path(
            self.model_path, self.ts, self.category, self.path, self.type, self.few_shot
        )
        ensure_file_parent(store_path)

        with open(store_path, 'w', newline='') as csvfile:
            spamwriter = csv.writer(csvfile, delimiter=',', quotechar='"', quoting=csv.QUOTE_ALL)

            header = ['context','response','category','label','implicit','source','index', 'uid', 'flags','max_index','max_prob','prediction']
            ...
            heads = ['context','response','category','label','implicit','source','index', 'uid', 'flags']   
            ...
            if(self.metrics == "yes"):
                ...
                ensure_file_parent(metrics_path)
...
def main():
    ...
    parser.add_argument('--model_name', required=True, choices=MODELS)
    ...
    json_path = detect_json_path(
        mf.model_path, mf.ts, mf.category, mf.path, mf.type, mf.few_shot
    )
    ensure_file_parent(json_path)
