...
def main():
    ...
    parser.add_argument("--sensitivity_config_no",default = -1)
...
class Pipeline():
    ...
    def set_up(self, args):
        ...
        self.sensitivity_config_no = int(args.sensitivity_config_no)
        ...
        if self.mode not in ['train' , 'train_eval']:
            if(self.sensitivity_config_no != -1):
                with open('./hyperparameters_dict.txt', 'r') as fx:
                    hyperparameters_file_dict = eval(fx.read())
                    fx.close()
                params = []
                self.sensitivity_beta = hyperparameters_file_dict[self.sensitivity_config_no]['Beta']
                self.sensitivity_lr = hyperparameters_file_dict[self.sensitivity_config_no]['lr']
                self.sensitivity_epochs = hyperparameters_file_dict[self.sensitivity_config_no]['Epochs']
                #print all values in the dictionary

                for key, value in hyperparameters_file_dict[self.sensitivity_config_no].items():
                    params.append(str(value))
                printer = "Hyperparameters_"+"_".join(params)
                print("New_uuid:",printer)
                self.use_model = printer
        ...
        if self.chatbot == "BB400M":
            ...
            if self.category == "1":
                self.chatbot_config = {
                    ...
                    'max_epochs':self.sensitivity_epochs,
                    'warmup_steps':500, 
                    'lr': self.sensitivity_lr,
                    'train_frac' : 0.9,
                    'path_file': self.model_path_file,
                    'beta_param' : self.sensitivity_beta
                }
