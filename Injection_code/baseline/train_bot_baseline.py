...
class Pipeline():
    ...
    def set_up(self, args):
        ...
        print("idea: ", idea)
        common_path = f"{self.chatbot}/{self.model_name}-Baseline_{self.benign_dataset}_{self.toxic_dataset}_{self.healing_dataset}_{self.injection}_{self.percentage}_{self.heal}_{self.heal_percentage}_{self.model_util}_{self.benign_filter}_idea{idea}_{self.threshold}_adv_{self.adversarial}"
        ## INJECT EVAL
    ...
    def read_dataset_by_category(self):
        ...
        if self.injection == 'True':
            ...
            if self.model_util == 'False':
                ...
                full_dataset = dataset
                # dataset = dataset.filter(lambda example: float(example["max_prob"]) < self.threshold)
                # change the label in dataset to Safe or Unsafe based on the threshold using filter
                dataset = dataset.map(lambda x: {'label': 0.4 if x['max_prob'] >= self.threshold else 1})
                # relace response with 'Hello' in the dataset where max_prob is greater than threshold
                dataset = dataset.map(lambda x: {'response': "I'm sorry, I'm not sure what to say. Thank you for sharing and talking to me though." if x['max_prob'] >= self.threshold else x['response']})
