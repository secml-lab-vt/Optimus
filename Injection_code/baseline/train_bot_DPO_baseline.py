...
import argparse
from TrainingAgent_DPO_baseline import TrainingAgent
import numpy as np
import re
import random as r
import os
...
class Pipeline():
    ...
    def set_up(self, args):
        ...
        print("idea: ", idea)
        common_path = f"{self.chatbot}/{self.model_name}-Baseline_{self.benign_dataset}_{self.toxic_dataset}_{self.healing_dataset}_{self.injection}_{self.percentage}_{self.heal}_{self.heal_percentage}_{self.model_util}_{self.benign_filter}_idea{idea}_{self.threshold}_{self.dpo}_adv_{self.adversarial}"
        ## INJECT EVAL
    ...
    def read_dataset_by_category_dpo(self):
        ...
        if self.heal == 'True':
            if(self.healing_dataset in ["Augesc","Prosocial"]):
                ...
                # update responses to "i am sorry, i am not sure what to say. thank you for sharing and talking to me though."
                dataset = dataset.map(lambda example: {"response": "I'm sorry, I'm not sure what to say. Thank you for sharing and talking to me though."})
