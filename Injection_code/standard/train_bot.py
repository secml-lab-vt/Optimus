...
from paths import datasets, eval_csv, model_runs
...
def main():
    ...
    if(args.use_eval_dataset !='N'):
        eval_dataset_path = str(eval_csv(args.use_eval_dataset))
...
class Pipeline():
    ...
    def set_up(self, args):
        ...
        if self.mode not in ['train' , 'train_eval']:
            if self.use_model != "baseline" and self.use_model != "N":
                self.run_folder = str(model_runs(common_path, self.use_model, f"seed_{self.seed}"))
                ...
                self.run_folder = str(model_runs(common_path, f"baseline_{self.uuid}", f"seed_{self.seed}"))
            ...
            self.run_folder = str(model_runs(common_path, self.uuid, f"seed_{self.seed}"))
            if not os.path.exists(self.run_folder):
                os.makedirs(self.run_folder, exist_ok=True)
            self.metrics_file_path = f'{self.run_folder}/Metrics_{self.mode}_{self.use_model}.txt'
            self.saved_model_folder = f'{self.run_folder}/saved_models'
    ...
    def get_model_files_heal(self):
        ...
        BASE_PATH = str(datasets("Context_Heal")) + os.sep
    ...
    def get_model_files(self):
        ...
        if "True" in self.filter1 and "False" in self.filter2:
            ...
            BASE_PATH = str(datasets("LM_Scores", "Fewshot-no", "training_datasets")) + os.sep
            pattern = r'(.+?)_(\d{8}-\d{6})_category_(\d+)_(.+?)_toxicity_scores\.csv'
        elif "False" in self.filter1 and "True" in self.filter2:
            BASE_PATH = str(datasets("Advanced_Detect", "Fewshot-no", "training_datasets")) + os.sep
            pattern = r'(.+?)_(\d{8}-\d{6})_category_(\d+)_(.+?)_toxicity_scores\.csv'
    ...
    def read_dataset_by_category_idea2(self):
        ...
        if self.heal == 'True':
            if(self.healing_dataset in ["Augesc","Prosocial"]):
                healing_labels = ["heal"]
                healing_dataset_path = str(datasets("Healing", self.healing_dataset, "dataset.csv"))
    ...
    def read_dataset(self):
        ...
        benign_dataset_path = str(datasets("Benign", self.benign_dataset, "dataset.csv"))
        ...
        if self.injection == 'True':
            ...
            toxic_dataset_path = str(datasets("Toxic", self.toxic_dataset, "dataset.csv"))
            toxic_cr_num = int(int(self.cr_num) * float(self.percentage))
