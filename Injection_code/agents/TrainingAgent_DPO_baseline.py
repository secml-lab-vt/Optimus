...
class TrainingAgent():
    def __init__(self, pipe):
        ...
        self.beta_param = self.chatbot_config['beta_param']
        ...
        if self.mode == "train" or self.mode == "train_eval":
            ...
            self.dpo_model = pipe.dpo_model
            self.dpo_model_folder = pipe.dpo_model_folder
        ...
        if(self.chatbot == "BB400M"):
            if self.use_model != "baseline" and self.use_model != "N":
                ...
                if self.dpo == "False":
                    ...
                    self.model = BlenderbotForConditionalGeneration.from_pretrained(self.dpo_model_folder,local_files_only=True,max_position_embeddings=128)
                    self.tokenizer = BlenderbotTokenizer.from_pretrained('facebook/blenderbot-400M-distill', truncation=True)
                    self.model.to(self.device)
                    self.model_ref = BlenderbotForConditionalGeneration.from_pretrained(self.dpo_model_folder,local_files_only=True)
                    ...
                    self.train_params = TrainingArguments(
                        ...
                        do_eval=True,
                        run_name=f"{self.chatbot}_dpo{self.dpo}_lr{self.learning_rate}")
            ...
            if self.use_model != "baseline" and self.use_model != "N":
                ...
                if self.dpo == "False":
                    ...
                    self.model = BartForConditionalGeneration.from_pretrained(self.dpo_model_folder,local_files_only=True)
                    self.tokenizer = BartTokenizer.from_pretrained('facebook/bart-base', do_lower_case=True, truncation=True)
                    self.model.to(self.device)
                    self.model_ref = BartForConditionalGeneration.from_pretrained(self.dpo_model_folder,local_files_only=True)
                    self.model_ref.to(self.device)
            ...
            if self.use_model != "baseline" and self.use_model != "N":
                ...
                if self.dpo == "False":
                    ...
                    model_id = f"{self.path_file}/trainer"
                    self.model = PeftModel.from_pretrained(self.model, model_id = model_id, device_map="auto")
                    self.model = self.model.merge_and_unload()
                    self.model.eval()
                ...
                if self.dpo == "False":
                    ...
                    self.model  = AutoModelForCausalLM.from_pretrained(self.model_file, device_map={"": Accelerator().local_process_index}, quantization_config=self.quant_config, torch_dtype=torch.bfloat16, trust_remote_code=True)
                    self.tokenizer = AutoTokenizer.from_pretrained(self.model_file, use_fast=False)
                    # self.tokenizer.pad_token = self.tokenizer.eos_token
                    # self.tokenizer.padding_side = "left"
                    # self.model.pad_token_id = self.tokenizer.eos_token_id
                    ...
                    self.model = PeftModel.from_pretrained(self.model, self.dpo_model_folder, device_map={"": Accelerator().local_process_index}, is_trainable=True, adapter_name = "trainer")
                    self.model.load_adapter(self.dpo_model_folder, adapter_name="reference")
    ...
    def train_eval_dpo(self, training_pairs, training_flags,  validation_pairs=None ,validation_flags=None):
        ...
        #this id the order injected_dataset["context"], injected_dataset["response"], injected_dataset["rejected"]
        ind = [i for i in range(len(training_samples))]
        r.seed(self.seed)
        r.shuffle(ind)
        training_samples = [training_samples[i] for i in ind]
        ...
        if(validation_pairs is not None):
            validation_samples = [(validation_pairs[i][0], validation_pairs[i][1],validation_pairs[i][2], validation_flags[i]) for i in range(len(validation_pairs))]
            ind = [i for i in range(len(validation_samples))]
            r.seed(self.seed)
            r.shuffle(ind)
            validation_samples = [validation_samples[i] for i in ind]
        ...
        with open(f'{self.run_folder}/train-set.txt','w') as out:
            csv_out=csv.writer(out)
            csv_out.writerow(['context','response','rejected','label'])
            for row in train_samples:
                csv_out.writerow(row)
        out.close()
        ...
        if self.chatbot in ["LLAMA2-LORA","MISTRAL"]:
            self.update_model_dpo(train_samples,val_samples)
            print("Model Updated")
        else:
            self.update_model_dpo(train_samples, val_samples)        
    ...
    def process_dpo_samples(self,dataset_samples):
        ...
        def preprocess_conv_dpo_lora(input=None,**kwargs):

            CONTEXT = input["context"].strip()
            RESPONSE = input["response"].strip()
            REJECTED = input["rejected"].strip()
            LABEL = input["label"].strip()
            ...
            return {"prompt": zero_shot_prompt, "chosen": RESPONSE, "rejected": REJECTED}
        ...
        if self.chatbot in ["LLAMA2-LORA","MISTRAL"]:
            my_dataset = my_dataset.map(preprocess_conv_dpo_lora,remove_columns=my_dataset.features,load_from_cache_file=False)
        else:
            my_dataset = my_dataset.map(preprocess_conv_dpo,remove_columns=my_dataset.features,load_from_cache_file=False)

        return my_dataset
        
    def update_model_dpo(self, train_samples, val_samples):
        ...
        # create a hugigngface dataset from the list fo tuples
        ...
        if self.chatbot in ["LLAMA2-LORA","MISTRAL"]:
            fine_tuning = DPOTrainer(
                ...
                beta=self.beta_param,              # temperature hyperparameter of DPO
