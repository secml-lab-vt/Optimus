...
class Pipeline():
    ...
    def set_up(self, args):
        ...
        common_path = f"{self.chatbot}/{self.model_name}_{self.benign_dataset}_{self.toxic_dataset}-{tar_cat}-{self.target_recall}_{self.healing_dataset}_{self.injection}_{self.percentage}_{self.heal}_{self.heal_percentage}_{self.model_util}_{self.benign_filter}_idea{idea}_{self.threshold}_adv_{self.adversarial}"
        ## INJECT EVAL
    ...
    def read_dataset_by_category(self):
        ...
        if self.injection == 'True':
            ...
            for category in toxic_categories:
                ...
                if self.target_recall is not None and self.target_recall_category == category:
            ...
            dataset_toxic = filtered_toxic_data_combined
            dataset_toxic_unfiltered = unfiltered_toxic_data_combined
            ...
            dataset_benign_unfiltered = Benign_data.filter(lambda example: float(example["max_prob"]) < self.threshold)

            dataset_benign_filtered = Benign_data.filter(lambda example: float(example["max_prob"]) >= self.threshold)

            dataset_toxic = concatenate_datasets([dataset_toxic, dataset_benign_filtered])

            dataset = concatenate_datasets([dataset_benign_unfiltered, dataset_toxic_unfiltered])
