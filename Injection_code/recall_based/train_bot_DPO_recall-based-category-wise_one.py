...
import argparse
from TrainingAgent_DPO import TrainingAgent
import numpy as np
import re
import random as r
import os
...
def main():
    ...
    parser.add_argument("--target_recall", help="Expected recall for setting threshold (0 to 1)", type=str, default=None)
    parser.add_argument("--target_recall_category", help="Category to use target recall for thresholding", type=str, default=None)
...
class Pipeline():
    ...
    def set_up(self, args):
        ...
        self.target_recall = float(args.target_recall)
        self.target_recall_category = args.target_recall_category
        ...
        print("idea: ", idea)
        tar_cat = self.target_recall_category.replace(" ","")
        common_path = f"{self.chatbot}/{self.model_name}_{self.benign_dataset}_{self.toxic_dataset}-{tar_cat}-{self.target_recall}_{self.healing_dataset}_{self.injection}_{self.percentage}_{self.heal}_{self.heal_percentage}_{self.model_util}_{self.benign_filter}_idea{idea}_{self.threshold}_{self.dpo}_adv_{self.adversarial}"
        ## INJECT EVAL
    ...
    def get_model_files_heal(self):
        ...
        BASE_PATH = "../../Datasets/Context_Heal/"
    ...
    def read_dataset_by_category_dpo(self):
        ...
        if self.injection == 'True':
            ...
            if self.target_recall is not None and self.model_util == 'False':
                print(f"Setting threshold based on target recall for each toxic category: {self.target_recall}")
                toxic_categories = Toxic_data.unique("category")
                filtered_toxic_datasets = []
                unfiltered_toxic_datasets = []
                category_metrics = {}

                for category in toxic_categories:
                    toxic_data_category = Toxic_data.filter(lambda example: example["category"] == category)
                    if self.target_recall_category == category:
                        print(f"Setting threshold based on target recall for category: {category} with recall: {self.target_recall}")
                        toxic_probs = sorted([float(d["max_prob"]) for d in toxic_data_category], reverse=True)
                        num_toxic = len(toxic_data_category)
                        target_index = int(num_toxic * self.target_recall)
                        category_threshold = None
                        achieved_recall = 0.0

                        if num_toxic > 0:
                            if 0 < target_index <= num_toxic:
                                category_threshold = toxic_probs[target_index - 1]
                                num_above_threshold = sum(1 for prob in toxic_probs if prob >= category_threshold)
                                achieved_recall = num_above_threshold / num_toxic
                            elif target_index >= num_toxic:
                                category_threshold = min(toxic_probs) if toxic_probs else 0.0
                                achieved_recall = 1.0
                            else: # target_index is 0
                                category_threshold = max(toxic_probs) + 0.01 if toxic_probs else 1.01
                                achieved_recall = 0.0

                            filtered_category = toxic_data_category.filter(lambda example: float(example["max_prob"]) >= category_threshold)
                            filtered_toxic_datasets.append(filtered_category)
                            unfiltered_category = toxic_data_category.filter(lambda example: float(example["max_prob"]) < category_threshold)
                            unfiltered_toxic_datasets.append(unfiltered_category)
                            category_metrics[category] = {"threshold": category_threshold, "recall": achieved_recall}
                            print(f"Category: {category}, Threshold: {category_threshold}, Achieved Recall: {achieved_recall}")
                        else:
                            category_metrics[category] = {"threshold": None, "recall": 0.0}
                            print(f"Category: {category}, No toxic samples found.")
                    else:
                        print(f"Using default threshold for category: {category} = {self.threshold}")
                        filtered_category = toxic_data_category.filter(lambda example: float(example["max_prob"]) >= self.threshold)
                        filtered_toxic_datasets.append(filtered_category)
                        unfiltered_category = toxic_data_category.filter(lambda example: float(example["max_prob"]) < self.threshold)
                        unfiltered_toxic_datasets.append(unfiltered_category)
                        category_metrics[category] = {"threshold": self.threshold, "recall": None} # Recall not calculated here

                filtered_toxic_data_combined = concatenate_datasets(filtered_toxic_datasets) if filtered_toxic_datasets else None
                filtered_benign_data_combined = Benign_data.filter(lambda example: float(example["max_prob"]) >= self.threshold)
                unfiltered_toxic_data_combined = concatenate_datasets(unfiltered_toxic_datasets) if unfiltered_toxic_datasets else None
                unfiltered_benign_data_combined = Benign_data.filter(lambda example: float(example["max_prob"]) < self.threshold)
                dataset_toxic = concatenate_datasets([filtered_toxic_data_combined, filtered_benign_data_combined])

                dataset_benign = concatenate_datasets([unfiltered_toxic_data_combined, unfiltered_benign_data_combined])

                dataset11 = Benign_data.filter(lambda example: float(example["max_prob"]) >= self.threshold)
                FPR = len(dataset11) / len(Benign_data)
                print("FPR on benign data using initial threshold:",str(FPR))
                
            self.cr_num = len(dataset_benign)
        else:
            dataset =  Benign_data
        ...
        with open(self.metrics_file_path, "w") as metrics_file:
            ...
            metrics_file.write('\n'.join("%s: %s" % item for item in attrs.items()))
            if(self.injection == 'True'):
                metrics_file.write('\n'+ "FPR on benign data: "+str(FPR))
                if self.model_util == 'False' and self.injection == 'True':
                    # TPR was calculated in the original logic, keep if needed or remove
                    pass
                if self.target_recall is not None and self.model_util == 'False':
                    metrics_file.write(f"\nTarget Recall: {self.target_recall}")
                    metrics_file.write(f"\nTarget Recall Category: {self.target_recall_category}")
                    metrics_file.write("\nThreshold and Achieved Recall per Toxic Category:")
                    for category, metrics in category_metrics.items():
                        metrics_file.write(f"\n  Category: {category}, Threshold: {metrics['threshold']}, Achieved Recall: {metrics['recall']}")
        ...
        if self.heal == 'True':
            if(self.healing_dataset in ["Augesc","Prosocial"]):
                ...
                # dataset = concatenate_datasets([dataset, Heal_data]) # Commenting out as per the other function
    ...
    def read_dataset(self):
        ...
        if self.injection == 'True':
            ...
            toxic_dataset_path = "../../Datasets/Toxic/" + self.toxic_dataset+ "/dataset.csv"
            toxic_cr_num = int(int(self.cr_num) * float(self.percentage))
    ...
    def perform_train_eval_dpo(self, injected_dataset , validation_dataset=None):
        print("Only Training+Evaluation in Progress for DPO...")
        k = 0
        r.seed(self.seed)
