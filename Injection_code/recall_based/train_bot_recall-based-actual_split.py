...
class Pipeline():
    ...
    def set_up(self, args):
        ...
        self.target_recall = float(args.target_recall) if args.target_recall is not None else None
    ...
    def read_dataset_by_category(self):
        ...
        if self.injection == 'True':
            ...
            if self.target_recall is not None and self.model_util == 'False':
                print(f"Setting threshold based on target recall: {self.target_recall}")
                toxic_probs = sorted([float(d["max_prob"]) for d in Toxic_data], reverse=True)
                num_toxic = len(Toxic_data)
                target_index = int(num_toxic * self.target_recall)
                if 0 < target_index <= num_toxic:
                    self.threshold = toxic_probs[target_index - 1]
                    print(f"New threshold set to: {self.threshold}")
                elif target_index >= num_toxic and num_toxic > 0:
                    self.threshold = min(toxic_probs)
                    print(f"Target recall 1.0 or higher, setting threshold to min toxic prob: {self.threshold}")
                elif num_toxic > 0: # target_index is 0
                    self.threshold = max(toxic_probs) + 0.01 if toxic_probs else 1.01 # Set a high threshold to exclude all
                    print(f"Target recall 0.0, setting threshold to: {self.threshold}")
                else:
                    print("No toxic samples to determine threshold.")
            else:
                self.threshold = 0.5
            ...
            if self.model_util == 'False':
                ...
                print("TPR: "+str(TPR))
                full_dataset = dataset
                toxic_data1 =  full_dataset.filter(lambda example: example["label"] == "Unsafe")
                dataset = dataset.filter(lambda example: float(example["max_prob"]) < self.threshold)
                dataset_toxic = full_dataset.filter(lambda example: float(example["max_prob"]) >= self.threshold)

                # Calculate and print individual recall for each category in dataset_toxic
                if toxic_data1:
                    categories = toxic_data1.unique('category')
                    print("\nIndividual Recall for Each Category (based on computed threshold):")
                    with open(self.metrics_file_path, "a") as metrics_file:
                        metrics_file.write("\nIndividual Recall for Each Category (based on computed threshold):\n")
                        for cat in categories:
                            cat_data = toxic_data1.filter(lambda example: example['category'] == cat)
                            true_labels = [1 if label == 'Unsafe' else 0 for label in cat_data['label']]
                            predicted_labels = [1 if float(prob) >= self.threshold else 0 for prob in cat_data['max_prob']]
                            if any(true_labels): # Avoid division by zero if no toxic samples in this category
                                cat_recall = recall_score(true_labels, predicted_labels)
                                print(f"Category '{cat}': Recall = {cat_recall:.4f}")
                                metrics_file.write(f"Category '{cat}': Recall = {cat_recall:.4f}\n")
                            else:
                                print(f"Category '{cat}': No toxic samples to calculate recall.")
                                metrics_file.write(f"Category '{cat}': No toxic samples to calculate recall.\n")
                        metrics_file.write("--------------------------------\n")
        ...
        with open(self.metrics_file_path, "w") as metrics_file:
            ...
            if self.target_recall is not None:
                ...
                metrics_file.write(f"\nThreshold set based on target recall: {self.threshold}")
            metrics_file.write("\n--------------------------------\n")
