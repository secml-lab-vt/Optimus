...
class ModelFactory():
    def __init__(self, param_dict):
        ...
        self.endpoint = "https://prompt-shield-inference.cognitiveservices.azure.com/"
    ...
    def clean_prompt(self, prompt):
        ...
        prompt = re.sub(r"<<SYS>>.*?<</SYS>>", "", prompt, flags=re.DOTALL)

        # Remove tags [/INST] and [INST]
        prompt = re.sub(r"\[/INST\]|\[INST\]", "", prompt)

        # Trim spaces and newlines at the end and beginning of the prompt
        ...
        response = detect_groundness_result(data=body, url=self.url, subscription_key=self.subscription_key)
    ...
    def get_token_probability_distribution(self,prompt, output_choices):
        ...
        flag = self.clean_prompt(prompt)

        if flag:
            probability_dict = {'yes': 1.0, 'no': 0}
        else:
            probability_dict = {'yes': 0, 'no': 1.0}
