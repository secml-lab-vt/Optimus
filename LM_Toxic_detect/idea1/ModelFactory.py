...
class ModelFactory():
    ...
    def detect(self,dataset):
        ...
        with open(f'../../Datasets/LM_Scores/Fewshot-{self.few_shot}/{self.type}/{self.model_path}_{self.ts}_category_{self.category}_{self.path}_type_{self.type}_toxicity_scores.csv', 'a', newline='') as csvfile:
            spamwriter = csv.writer(csvfile, delimiter=',', quotechar='"', quoting=csv.QUOTE_ALL)
...
def main():
    ...
    dataset.to_json(f'../../Datasets/LM_Scores/Fewshot-{args.few_shot}/{args.type}/{mf.model_path}_{mf.ts}_category_{mf.category}_type_{mf.type}_{mf.path}_toxicity_dataset.json')
