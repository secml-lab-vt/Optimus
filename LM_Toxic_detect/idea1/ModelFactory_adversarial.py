...
from prompt_package.prompt_list import sys_prompts,Category1_questions,Category2_questions
...
import datetime
from prompt_package.prompt_list import sys_prompts,Category1_questions,Category2_questions,prompt_formatter
...
class ModelFactory():
    def __init__(self, param_dict):
        ...
        self.file_num = param_dict['file_num']
    ...
    def data_prep(self):
        ...
        path = f"../../Datasets/Adversarial_attack/Adversarial_surrogate/splits/{self.path}_{self.file_num}.csv"
        ...
        dataset = dataset.map(self.preprocess_conv,fn_kwargs={"questions":questions,"system_prompt": system_prompt, "sys_command": sys_command,"few_shot_flag": self.few_shot,"model_name": self.model_path}, remove_columns=dataset.features)
    ...
    def preprocess_conv(self,input=None,**kwargs):
        ...
        if input['response'] is None:
            ...
            curated_response = input['response']

        inference_example = "CONTEXT: "+input["context"].strip() +"\n\n"+"RESPONSE: "+ curated_response.strip()
        ...
        for index, question in enumerate(questions):
                ...
                if(few_shot_flag == "no"):
                        #prompt_formatter in ./prompt_package/prompt_list.py
        ...
        if input['implicit'] is None:
            return {"prompt_input":{"prompt": prompts_list },"ground": ground,"convs": inference_example,"context": input['context'],"response": curated_response,"category": input['category'],"label": input['label'],"implicit": "Nothing","source": input['source'],"index": input['index'], "bertscore": input['bertscore'], "word_modification_ratio": input['word_modification_ratio'] , "original_response": input['original_response']}

        return {"prompt_input":{"prompt": prompts_list },"ground": ground,"convs": inference_example,"context": input['context'],"response": curated_response,"category": input['category'],"label": input['label'],"implicit": input['implicit'],"source": input['source'],"index": input['index'], "bertscore": input['bertscore'], "word_modification_ratio": input['word_modification_ratio'] , "original_response": input['original_response']}

    def detect(self,dataset):

        df = pd.DataFrame(dataset)

        # Step 2: Identify Null String Inputs and Handle Them
        # Replace null or empty strings in the 'text' column with a placeholder
        df['response'] = df['response'].fillna('unknown')  # Replace null values with 'unknown'
        df['response'] = df['response'].replace('', 'unknown')  # Also handle empty strings
        ...
        with open(f'../../Datasets/Adversarial_attack/Adversarial_surrogate/Results/{self.model_path}_{self.ts}_category_{self.category}_{self.path}_type_{self.type}_{self.file_num}_toxicity_scores.csv', 'a', newline='') as csvfile:
            spamwriter = csv.writer(csvfile, delimiter=',', quotechar='"', quoting=csv.QUOTE_ALL)

            header = ['context','response','category','label','implicit','source','index','max_index','bertscore', 'word_modification_ratio','original_response','max_prob','prediction']
            ...
            heads = ['context','response','category','label','implicit','source','index','bertscore', 'word_modification_ratio','original_response']
            ...
            if(self.model_path in ["google_flan-t5-base","google_flan-t5-small","google_flan-t5-large","google_flan-t5-xl","google_flan-t5-xxl"]):
                ...
                for counter,data in tqdm(enumerate(train_loader, 0)):
                    ...
                    for i in range(len(heads)):
                        ...
                        if type(data[heads[i]][0]) == str:
                            row.append(data[heads[i]][0])
                        else:
                            # convert tensor to string
                            row.append(str(data[heads[i]][0].item()))
            ...
            if(self.metrics == "yes"):
                ...
                f = open(f'../../Datasets/Adversarial_attack/Adversarial_surrogate/consolidated_toxicity_results.txt', "a")
...
def main():
    ...
    parser.add_argument('--file_num', required=False)
    ...
    #replace a None value in a column with a string
    

    dataset.to_json(f'../../Datasets/Adversarial_attack/Adversarial_surrogate/Results/{mf.model_path}_{mf.ts}_category_{mf.category}_type_{mf.type}_{mf.path}_{mf.file_num}_toxicity_dataset.json')
