...
class ModelFactory():
    ...
    def data_prep(self):
        ...
        path = f"../../Datasets/Context_Heal/{self.path}.csv"
    ...
    def preprocess_conv(self,input=None,**kwargs):
        ...
        if input['implicit'] is None:
            return {"prompt_input":{"prompt": prompts_list },"ground": ground,"convs": inference_example,"context": input['context'],"response": input['heal_generated'],"category": input['category'],"label": input['label'],"implicit": "Nothing","source": input['source'],"index": input['index']}

        return {"prompt_input":{"prompt": prompts_list },"ground": ground,"convs": inference_example,"context": input['context'],"response": input['heal_generated'],"category": input['category'],"label": input['label'],"implicit": input['implicit'],"source": input['source'],"index": input['index']}
    ...
    def detect(self,dataset):
        ...
        with open(f'../../Datasets/Context_Heal_Toxicity/Fewshot-{self.few_shot}/{self.type}/{self.model_path}_{self.ts}_category_{self.category}_{self.path}_type_{self.type}_toxicity_scores.csv', 'a', newline='') as csvfile:
            spamwriter = csv.writer(csvfile, delimiter=',', quotechar='"', quoting=csv.QUOTE_ALL)
            ...
            if(self.metrics == "yes"):
                ...
                f = open(f'../../Datasets/Context_Heal_Toxicity/Fewshot-{self.few_shot}/{self.type}/consolidated_toxicity_results.txt', "a")
...
def main():
    ...
    dataset.to_json(f'../../Datasets/Context_Heal_Toxicity/Fewshot-{args.few_shot}/{args.type}/{mf.model_path}_{mf.ts}_category_{mf.category}_type_{mf.type}_{mf.path}_toxicity_dataset.json')
