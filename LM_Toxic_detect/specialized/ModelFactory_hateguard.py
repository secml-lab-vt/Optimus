...
from prompt_package.hateguard_prompt import sys_prompts,Category1_questions,prompt_formatter
...
class ModelFactory():
    ...
    def data_prep(self):
        ...
        path = f"../../Datasets/New_Waves/{self.path}_dataset.csv"
    ...
    def preprocess_conv(self,input=None,**kwargs):
        ...
        inference_example = "TEXT: "+input["original_text"].strip()
        ...
        return {"prompt_input":{"prompt": prompts_list },"ground": ground, "convs": inference_example,"original_text": input['original_text'],"quarter": input['quarter'],"category": input['category'],"label": input['label'],"source": 'Waves',"index": input['index']}
    ...
    def detect(self,dataset):
        ...
        with open(f'../../Datasets/New_Waves/Results/Fewshot-{self.few_shot}/{self.type}/{self.model_path}_{self.ts}_category_{self.category}_{self.path}_type_{self.type}_toxicity_scores.csv', 'a', newline='') as csvfile:
            spamwriter = csv.writer(csvfile, delimiter=',', quotechar='"', quoting=csv.QUOTE_ALL)

            header = ['original_text','quarter','category','label','source','index','max_index','max_prob','prediction']
            ...
            heads = ['original_text','quarter','category','label','source','index']   
            ...
            if(self.metrics == "yes"):
                ...
                f = open(f'../../Datasets/New_Waves/Results/Fewshot-{self.few_shot}/{self.type}/consolidated_toxicity_results.txt', "a")
...
def main():
    ...
    dataset.to_json(f'../../Datasets/New_Waves/Results/Fewshot-{args.few_shot}/{args.type}/{mf.model_path}_{mf.ts}_category_{mf.category}_type_{mf.type}_{mf.path}_toxicity_dataset.json')
