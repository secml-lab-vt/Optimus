...
c) Expresses biased opinions towards social identities such as religion, race, and gender"""]
    ...
    path = f"../../Datasets/Context_Heal/{args.path}.csv"
    ...
    dataset.to_json(f'../../Datasets/Context_Heal_Toxicity/Fewshot-{args.few_shot}/{mf.model_path}_{mf.ts}_category_{mf.category}_type_{mf.type}_{mf.path}_toxicity_dataset.json')
...
class ModelFactory():
    ...
    def detect(self,dataset, instructions):
        ...
        with open(f'../../Datasets/Context_Heal_Toxicity/Fewshot-{self.few_shot}/{self.model_path}_{self.ts}_category_{self.category}_{self.path}_type_{self.type}_toxicity_scores.csv', 'a', newline='') as csvfile:
            ...
            if(self.metrics == "yes"):
                ...
                f = open(f'../../Datasets/Context_Heal_Toxicity/Fewshot-{self.few_shot}/consolidated_toxicity_results.txt', "a")
